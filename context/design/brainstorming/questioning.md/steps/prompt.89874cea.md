---
timestamp: 'Sun Oct 19 2025 21:18:06 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251019_211806.c93cf41f.md]]'
content_id: 89874cea1e66b82ab23ca1b15588380fdccc28ab92e99ed7c363f4fee5197bbb
---

# prompt: Modify the following Deno test suite to use assertThrows from "jsr:@std/assert" whenever a test is checking that an error should be thrown, using the error types that are defined in errors.ts adn thrown by the src files. This is the test suite to modify:
...
Deno.test("Inventory Viewer Queries", async (t) => {
  ...
  try {
    await fs.copyFile(srcInventory, tmpInventory); // Use Deno.copyFile
...
Deno.test("Reservation System Flow", async (t) => {
  ...
  try {
    ...
    await t.step(
      "Edge Case: Checkout with invalid kerb throws error",
      async () => {
        ...
        assert(
          badKerbThrew,
          "checkoutItem should fail for a missing kerb (user)",
        );
      },
    );

    await t.step(
      "Edge Case: Checkout non-existent item throws error",
      async () => {
    ...
    await t.step(
      "Edge Case: Checkin non-reserved item throws error",
      async () => {
...
Deno.test("Concurrent Operations & LLM Integration", async (t) => {
  ...
  try {
    ...
    await t.step(
      "First concurrent checkout succeeds, second fails",
      async () => {
