---
timestamp: 'Sun Oct 19 2025 20:56:24 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251019_205624.3b549e28.md]]'
content_id: 2a1f0879e93380b4f65631ae2f2a7bd06db45425dd9c970d71c212a62c0a36bf
---

# response:

To reformat the Deno test suite as requested, we'll make the following changes:

1. **Define `setupTestFiles()` and `teardownTestFiles()`:** These functions will be placed at the top level of the file. They will manage the creation and deletion of the temporary CSV files used by the `Reservation` class tests.
2. **Integrate `setupTestFiles()` and `teardownTestFiles()` into CSV Suite:** The `Deno.test("Reservation System Test Suite (CSV-based)", ...)` block will call `setupTestFiles()` at its beginning and `teardownTestFiles()` at its end. The module-scoped `inventoryCsvPath` and `usersCsvPath` will be populated by `setupTestFiles()`.
3. **Keep `beforeEachStep()` for CSV Tests:** The `beforeEachStep()` helper within the CSV test suite is not a Deno-native `t.beforeEach()`. It's a manual helper that ensures each individual `t.step` gets a clean *content* state for the temporary CSV files. This is crucial for test isolation for file-based tests and does not violate the "remove `t.beforeEach()`" constraint.
4. **Remove `t.beforeEach()` and `t.afterEach()` from MongoDB Suite:** For the `Deno.test("InventoryReservation System Test Suite (MongoDB)", ...)` block, we will remove all instances of `t.beforeEach()` and `t.afterEach()`. To maintain test isolation, the setup logic (`testDb()`, `populateInitialData()`, `new InventoryReservation(db)`) and teardown logic (`client.close()`) will be moved directly into *each individual `t.step` function*. This results in more verbose, but isolated and compliant, test cases.
...
async function setupTestFiles(): Promise<TestFilePaths> {
  ...
  const tmpUsers = await Deno.makeTempFile({ prefix: "users_", suffix: ".csv" });
