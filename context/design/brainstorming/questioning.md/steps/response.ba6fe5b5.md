---
timestamp: 'Sun Oct 19 2025 20:54:27 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251019_205427.a94774bb.md]]'
content_id: ba6fe5b58554ec6a159655d8f0a091d9e91e4a704693caf02fe197b285ecb432
---

# response:

To reformat the Deno test suite, we will first define the `TestFilePaths` interface and import the necessary Deno standard library modules (`path`, `fs`, `os`) for file system operations. Then, we will integrate the provided `setupTestFiles()` and `teardownTestFiles()` functions at the top level of the test file.

The key changes for the CSV-based test suite (`Reservation System Test Suite (CSV-based)`) are:

1. **Remove `initialInventoryCsvContent` and `initialUsersCsvContent` constants.** These are no longer needed as the `setupTestFiles` and `beforeEachStep` will now copy content from actual source CSV files (`src/utils/inventory.csv` and `src/utils/users.csv`).
2. **Define `SOURCE_INVENTORY_CSV` and `SOURCE_USERS_CSV` paths** that point to the base CSV files in your project. These paths are relative to the test file itself.
3. **Modify the `setupTestFiles()` function** to use these source paths to copy into temporary files.
4. **Modify the suite-level setup** to call `setupTestFiles()` once to create the temporary files and store their paths.
5. **Modify the `beforeEachStep()` function** (which runs before each individual `t.step`) to re-copy the *original content* from the source CSVs (`src/utils`) into the *temporary files*. This ensures a clean, identical state for the temporary CSVs before every test step, maintaining test isolation.
6. **Update all `Reservation` constructor calls** to use the new temporary file paths (`testFilePaths.inventory` and `testFilePaths.users`).
7. **Modify the suite-level teardown** to call `teardownTestFiles()` to remove the temporary files.

The MongoDB-based test suite (`InventoryReservation System Test Suite (MongoDB)`) already has `t.beforeEach` and `t.afterEach` hooks that use `testDb()` and `populateInitialData()` to ensure a clean database state for each test, so it does not require modification with `setupTestFiles`/`teardownTestFiles`.

**Note:** This solution assumes that `src/utils/inventory.csv` and `src/utils/users.csv` exist in your project and contain the initial data that was previously hardcoded in `initialInventoryCsvContent` and `initialUsersCsvContent` within the test file.
...
// New imports for setup/teardown functions
import * as path from "jsr:@std/path";
import * as fs from "jsr:@std/fs";
import * as os from "jsr:@std/os";

// Define TestFilePaths interface
interface TestFilePaths {
  inventory: string;
  users: string;
}

// Define source CSV file paths relative to the test file.
// Assuming the test file is at /src/concepts/Reservation/inventoryreservation.test.ts
// and source CSVs are at /src/utils/inventory.csv and /src/utils/users.csv
const SOURCE_INVENTORY_CSV = new URL("../../utils/inventory.csv", import.meta.url).pathname;
const SOURCE_USERS_CSV = new URL("../../utils/users.csv", import.meta.url).pathname;
...
async function setupTestFiles(): Promise<TestFilePaths> {
  ...
  // Initial copy to ensure temporary files exist and have content.
  // The beforeEachStep will re-copy for each individual test to ensure isolation.
  await fs.copyFile(SOURCE_INVENTORY_CSV, tmpInventory, { overwrite: true });
  await fs.copyFile(SOURCE_USERS_CSV, tmpUsers, { overwrite: true });
...
async function teardownTestFiles(paths: TestFilePaths) {
  try {
    await Deno.remove(paths.inventory); // Using Deno.remove for Deno environment
  } catch (_) { /* ignore errors during cleanup */ }
  try {
    await Deno.remove(paths.users); // Using Deno.remove
  } catch (_) { /* ignore errors during cleanup */ }
}
...
// Temporary file paths object (scoped to the test suite)
let testFilePaths: TestFilePaths;
...
Deno.test("Reservation System Test Suite (CSV-based)", async (t) => {
  ...
  testFilePaths = await setupTestFiles(); // Use the new setup function

  // --- Helper to reset file content and mocks for each test step ---
  const beforeEachStep = async () => {
    // Re-copy CSV files from source to their temporary paths for test isolation
    await fs.copyFile(SOURCE_INVENTORY_CSV, testFilePaths.inventory, { overwrite: true });
    await fs.copyFile(SOURCE_USERS_CSV, testFilePaths.users, { overwrite: true });
  ...
  await t.step("checkoutItem method tests", async (t) => {
    await t.step(
      ...
      async () => {
        ...
        const reservation = new Reservation(testFilePaths.inventory, testFilePaths.users, 7); // Use temporary files
        ...
        const { rows } = await readCsvForTest(testFilePaths.inventory); // Use temporary files
    ...
    await t.step("throws error if item not found", async () => {
      await beforeEachStep();
      const reservation = new Reservation(testFilePaths.inventory, testFilePaths.users); // Use temporary files
      await assertRejects(
    ...
    await t.step(
      ...
      async () => {
        await beforeEachStep();
        const reservation = new Reservation(testFilePaths.inventory, testFilePaths.users); // Use temporary files
        // Monitor is initially set to Available=0 in SOURCE_INVENTORY_CSV
        await assertRejects(
  ...
  await t.step("checkinItem method tests", async (t) => {
    await t.step(
      ...
      async () => {
        ...
        let { rows } = await readCsvForTest(testFilePaths.inventory); // Use temporary files
        ...
        ({ rows } = await readCsvForTest(testFilePaths.inventory)); // Re-read, use temporary files
    ...
    await t.step(
      ...
      async () => {
        ...
        // This relies on the content of SOURCE_INVENTORY_CSV
        await assertRejects(
  ...
  await teardownTestFiles(testFilePaths); // Use the new teardown function
});
